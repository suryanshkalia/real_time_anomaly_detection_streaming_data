...
class Graph:
        ...
        try:
            if not node.inputs: #producer node
                try:
                    await node.coro(
                        grph = self,
                        output_queue = self._fanout_queue(node),
                        ...
                        for attempt in range(node.retries):
                            try:
                                ...
                                if node.first_item_time is None:
                                    node.first_item_time = curr_time
                                node.last_item_time = curr_time
                                # done to get the onl;y time when workers are engaged

                                break

                            except Exception as e:
                                if attempt == node.retries - 1:
                                    #insert the node in dlq for later inspec.
            ...
            async def put(self, item):
                ...
                if data is STOP:
                    for out_id in self.node.outputs:
                        ...
                        for i in range(out_node.workers):
                            await out_node.input_queue.put(
                                (float("inf"), STOP)
                                )
                    return
    ...
    async def start(self):
        ...
        for node in self.nodes.values():
            worker_count = (
                node.workers if node.inputs else 1
            )  # only 1 producer

            for i in range(
                worker_count
            ):  # there are 3 workers for each node but should not be for producer
        ...
        for node in self.nodes.values():
            ...
            if node.first_item_time and node.last_item_time:
                time_elapsed = (
                    node.last_item_time - node.first_item_time
                )
        ...
        if times:
            ...
            print(
                f"system throughput: {system_throughput: .2f} items/sec"
            )
    ...
    async def monitor(self):
        while not self.stop_event.is_set():
            ...
            print(
                f"Global Pressure: {self.get_global_pressure():.2f}\n"
            )
    ...
    def calculate_sleep(self):
        ...
        self.current_sleep = max(
            self.min_sleep,
            min(self.max_sleep, self.current_sleep),
        )
